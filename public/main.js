...
// ✨ 기존 선생님 문제와 새로운 연습 문제를 모두 합친(Merge) 초대용량 데이터
const subjectData = {
    common1: {
        title: "공통수학1",
        ...
        standards: [
            { 
                ...
                questions: [
                    ...
                    // 추가된 문제
                    { q: "다항식 $P(x) = x^3 - 2x^2 + 3x - 4$와 $Q(x) = 2x^3 + x^2 - 5x + 1$에 대하여 $2P(x) - Q(x)$를 계산하시오.", level: "C", reason: "다항식의 실수배와 뺄셈 등 다항식의 연산을 복합적으로 수행하는 전형적인 문항입니다." },
                    { q: "모서리의 길이가 각각 $a, b, c$인 직육면체의 겉넓이가 $94$이고, 모든 모서리의 길이의 합이 $48$일 때, 이 직육면체의 대각선의 길이를 구하시오.", level: "B", reason: "다항식의 곱셈 공식 $(a+b+c)^2 = a^2+b^2+c^2+2(ab+bc+ca)$를 기하학적 상황에 적용하여 해결해야 합니다." },
                    { q: "두 다항식 $A = x^2+2x, B = 3x-1$일 때, $A+B$를 구하시오.", level: "D", reason: "기본적인 이차식과 일차식의 덧셈을 수행하는 기초 문항입니다." },
                    { q: "다항식의 곱셈에서 분배법칙을 이용하여 $(x+y)(x^2-xy+y^2)$을 전개하고, 이 전개 과정이 어떻게 공식 $x^3+y^3$으로 유도되는지 원리를 설명하시오.", level: "A", reason: "연산의 원리를 이해하고 이를 바탕으로 공식을 직접 유도 및 논리적으로 설명해야 합니다." }
                ]
            },
            { 
                code: "[10공수1-01-02]", 
                ...
                questions: [
                    ...
                    // 추가된 문제
                    { q: "다항식 $P(x)$를 $(x-1)^2$으로 나누었을 때의 나머지가 $2x+1$이고, $x-2$로 나누었을 때의 나머지가 $4$이다. $P(x)$를 $(x-1)^2(x-2)$로 나누었을 때의 나머지를 구하시오.", level: "A", reason: "나머지정리의 심화 응용으로, 몫과 나머지의 구조를 깊이 이해하고 미지수를 설정하여 풀어야 하는 고난도 문항입니다." },
                    { q: "다항식 $P(x) = x^3 + ax^2 + bx + 2$가 $x-1$과 $x+2$로 각각 나누어떨어질 때, 상수 $a, b$의 값을 구하시오.", level: "B", reason: "인수정리를 두 번 적용하여 연립방정식을 세우고 푸는 응용 문항입니다." },
                    { q: "등식 $ax^2 + (b-1)x + c = 2x^2 + 3x - 5$가 $x$에 대한 항등식일 때, 상수 $a, b, c$의 값을 구하시오.", level: "C", reason: "항등식의 성질(계수비교법)을 직접 적용하여 미지수를 구하는 기본 문항입니다." },
                    { q: "다항식 $P(x) = x^2 - 4x + 7$을 $x-2$로 나누었을 때의 나머지를 나머지정리를 이용하여 구하시오.", level: "D", reason: "나머지정리 $P(a) = R$ 공식을 직접적으로 단순 대입하는 기초 문항입니다." }
                ]
            },
            { 
                code: "[10공수1-01-03]", 
                ...
                questions: [
                    ...
                    // 추가된 문제
                    { q: "다항식 $x^4 - 13x^2 + 36$을 인수분해하시오.", level: "B", reason: "복이차식의 형태를 파악하고 $x^2 = X$로 치환하여 인수분해를 두 번 수행해야 합니다." },
                    { q: "다항식 $x^3 - 8$을 인수분해하시오.", level: "C", reason: "$a^3 - b^3$ 꼴의 인수분해 공식을 직접 적용하는 표준 문항입니다." }
                ]
            },
            { 
                code: "[10공수1-02-01]", 
                ...
                questions: [
                    ...
                    // 추가된 문제
                    { q: "복소수 $z = (x-2) + (x^2-4)i$에 대하여 $z^2$이 음의 실수가 되도록 하는 실수 $x$의 값을 구하시오.", level: "B", reason: "$z^2$이 음실수가 될 조건($z$가 순허수)을 이해하고 실수부와 허수부의 조건을 설정하여 풀어야 합니다." },
                    { q: "다음 복소수의 실수부분과 허수부분을 각각 구하시오: $3 - 5i$", level: "E", reason: "복소수의 기본 형태 $a+bi$에서 실수부와 허수부의 정의만 묻는 기초 문항입니다." }
                ]
            },
            { 
                code: "[10공수1-02-04]", 
                ...
                questions: [
                    ...
                    // 추가된 문제
                    { q: "이차함수 $y = x^2 - 4x + k$의 그래프가 $x$축과 서로 다른 두 점에서 만나도록 하는 실수 $k$의 값의 범위를 구하시오.", level: "C", reason: "함수의 x축 교점과 방정식의 실근 개수를 연결하여 판별식을 적용하는 문항입니다." },
                    { q: "직선 $y = 2x+1$과 이차함수 $y = x^2 + ax + b$의 그래프가 점 $(1, 3)$에서 접할 때, 상수 $a, b$의 값을 구하시오.", level: "B", reason: "이차함수와 직선의 위치관계(접함)를 판별식 $D=0$과 교점 좌표를 이용하여 연립해야 합니다." },
                    { q: "이차방정식 $x^2-2kx+k+2=0$의 두 근이 모두 $1$보다 클 때, 실수 $k$의 값의 범위를 이차함수의 그래프를 이용하여 구하고 그 논리를 서술하시오.", level: "A", reason: "근의 분리(근의 위치) 문제로, 판별식, 축의 위치, 경계에서의 함숫값을 모두 고려하여 논리적으로 설명해야 합니다." }
                ]
            }
        ]
    },
        ...
        standards: [
            ...
            },
            {
                ...
                questions: [
                    ...
                    // 추가된 문제
                    { q: "원 $x^2 + y^2 - 4x + 2y - 4 = 0$과 직선 $y = 2x+k$가 만나서 생기는 현의 길이가 $4$일 때, 양수 $k$의 값을 구하시오.", level: "A", reason: "원의 중심에서 직선까지의 거리(d), 반지름(r), 현의 길이(l)를 피타고라스 정리로 연결하는 고난도 기하 문제입니다." },
                    { q: "원 $x^2 + y^2 = 5$ 밖의 점 $P(3, 1)$에서 이 원에 그은 두 접선의 방정식을 각각 구하시오.", level: "B", reason: "원 밖의 점에서 그은 접선의 방정식을 판별식이나 점과 직선 사이의 거리 공식을 이용해 구하는 응용 문제입니다." }
                ]
            },
            {
                code: "[10공수2-02-03]",
...
async function startLevelMatching(code) {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            // ✨ 정규식을 최신 양식(성취수준: A~E)에 맞게 완벽 수정하여 수준이 변형되지 않도록 보호
